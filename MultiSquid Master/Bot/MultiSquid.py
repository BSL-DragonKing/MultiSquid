...
#os.chdir(r'G:\My Drive\MultiSquid Master\Bot')
...
async def helpj(ctx):
    if ctx.author.id in [507002318420574208, 661258412721438762]:
        ...
        helpj.add_field(name=f'@Jenny.#6969', value=f'**Pup** - Brings you puppy commands for when your sad 😭')
...
Wumpus = 'G:/My Drive/Pictures/Wumpus'
puppies = 'G:/My Drive/Pictures/Puppies'
pvifam = 'G:/My Drive/Pictures/PVI Friends/iCloud Photos'
...
async def pup(ctx):
    if ctx.author.id in [507002318420574208, 661258412721438762]:
        image = False
        directory = os.listdir(puppies)
        while not image:
            result = random.randint(0, len(directory) - 1)
            if '.ini' not in directory[result]: image = True
        path = f'{puppies}/{directory[result]}'
        f = discord.File(path)
